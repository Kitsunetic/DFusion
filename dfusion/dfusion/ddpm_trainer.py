...
class DDPMTrainer(DiffusionBase):
    ...
    def get_vlb(self, denoise_fn, x_start, x_t, t):
        ...
        out["vlb"] = th.where(t == 0, decoder_nll, kl)
        return out
    ...
    def forward(
        ...
        if self.loss_type in ("kl", "rescaled_kl"):
            # \mathcal L_\text{kl}
            info.update(self.get_vlb(denoise_fn, x_start, x_t, t))
            ...
            if self.model_var_type in ("learned", "learned_range"):
                assert model_out.size(1) == 2 * x_t.size(1), "output channel must be double of input channel for learned_*"
                model_out, tmp = th.chunk(model_out, chunks=2, dim=1)
                frozen_out = th.cat([model_out.detach(), tmp], dim=1)
                info.update(self.get_vlb(lambda *_: frozen_out, x_start, x_t, t))
                losses["vlb"] = info["vlb"]
...
def __test__():
    ...
    losses, info = diffusion_trainer(model, data, return_info=True)
    print(losses)
    ...
    # t : torch.Size([2])
    # eps : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # x_t : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # x_start : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # model_out : torch.Size([2, 6, 4, 5, 6, 7, 8, 9])
    # model_log_var : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # pred_eps : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # model_mean : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # pred_x_start : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # vlb : torch.Size([2])
    # x_prev : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
    # target : torch.Size([2, 3, 4, 5, 6, 7, 8, 9])
