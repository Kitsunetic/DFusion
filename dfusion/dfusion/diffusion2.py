...
class GaussianDiffusionSampler(nn.Module):
    ...
    def q_mean_variance(self, x_0, x_t, t):
        ...
        posterior_mean = (
            extract(self.posterior_mean_coef1, t, x_t.shape) * x_0 + extract(self.posterior_mean_coef2, t, x_t.shape) * x_t
        )
    ...
    def predict_xstart_from_eps(self, x_t, t, eps):
        ...
        return (
            extract(self.sqrt_recip_alphas_bar, t, x_t.shape) * x_t - extract(self.sqrt_recipm1_alphas_bar, t, x_t.shape) * eps
        )
    ...
    def forward(self, shape):
        ...
        Algorithm 2.
        """
        x_t = torch.randn(shape, dtype=torch.float, device=self.betas.device)
        for time_step in reversed(range(self.T)):
            t = x_t.new_ones([shape[0]], dtype=torch.long) * time_step
