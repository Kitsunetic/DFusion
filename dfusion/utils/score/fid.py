...
def get_statistics(images, num_images=None, batch_size=50, use_torch=False, verbose=False, parallel=False):
    ...
    if num_images is None:
        try:
            ...
            raise ValueError("when `images` is not a list like object (e.g. generator), " "`num_images` should be given")
...
def get_fid_score(stats_cache, images, num_images=None, batch_size=50, use_torch=False, verbose=False, parallel=False):
    m1, s1 = get_statistics(images, num_images, batch_size, use_torch, verbose, parallel)
